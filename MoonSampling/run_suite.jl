...
# State-Dependent Action Space: https://github.com/JuliaPOMDP/POMDPs.jl/discussions/353
function POMDPs.actions(mdp::ExtractionMDP, s::ExtractionState)
    if s.full == true
        return [1, 2]
    else
        return collect(3:(mdp.map_size[1]*mdp.map_size[2])+2) # no accept, reject actions
    end
end
...
function create_gif(maps_over_time, pos_over_time, action_over_time; filename="animation.gif", fps=2)
    anim = @animate for (i, frame) in enumerate(maps_over_time)
        heatmap(frame, color=:blues, legend=false, yflip=true, axis=false, ticks=true, aspect_ratio=:equal)
        color = :grey
...
truth_maps = [rand(Bool, 30, 30) for _ in 1:10]

# run solver for all truth maps
for truth_map in truth_maps
    time = @elapsed begin
    mdp = ExtractionMDP(SA[30,30], truth_map)
